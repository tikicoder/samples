...
& "$(Join-Path -Path $scriptPath_init -ChildPath "..\..\..\..\general_programming_scripting\powershell\vsCode\vsCodeManuallBackup.ps1" | Resolve-Path)" -isRestore $true -wsl_command $($general_defaults.main_distro)
...
wsl -d $($general_defaults.main_distro) bash "$($general_defaults.tmp_directory)/user_docker_init.sh" "$($general_defaults.docker_sock)" "$($general_defaults.docker_host_sock)" "$($general_defaults.tiki_docker_desktop)" "$($general_defaults.docker_dir)"


if (Test-Path "\\wsl$\$($general_defaults.docker_distro)$($general_defaults.tmp_directory)\user_init.sh"){Remove-Item -Path "\\wsl$\$($general_defaults.docker_distro)$($general_defaults.tmp_directory)\user_init.sh"}
Write-Host "Coping Script user_init.sh"
Copy-item -Path $(Join-Path -Path $general_defaults.root_path -ChildPath "general\wsl\scripts\user_init.sh") -Destination "\\wsl$\$($general_defaults.docker_distro)$($general_defaults.tmp_directory)\user_init.sh"
wsl -d $($general_defaults.docker_distro) bash "$($general_defaults.tmp_directory)/user_init.sh" 
