...
if(-not [string]::IsNullOrWhiteSpace($subscriptionIdFilter)){
  if($arrSubscriptionIdFilter.Length -gt 0){
    $arrSubscriptionIdFilter = $arrSubscriptionIdFilter.ToLower().split(",")
  } 
}
$arrSubscriptionIdExclude = $null
if(-not [string]::IsNullOrWhiteSpace($subscriptionIdExclude)){
  if($arrSubscriptionIdExclude.Length -gt 0){
    $arrSubscriptionIdExclude = $arrSubscriptionIdExclude.ToLower().split(",")
  } 
}
